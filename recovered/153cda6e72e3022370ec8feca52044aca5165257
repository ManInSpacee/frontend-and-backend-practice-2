...
socket.on('taskAdded', (task) => {
  console.log('WebSocket: задача от другого клиента:', task.text);
...
async function subscribeToPush() {
  ...
  const keyResponse   = await fetch('https://localhost:3001/vapid-public-key');
...
btnPushEnable.addEventListener('click', async () => {
  if (Notification.permission === 'denied') {
    alert('Уведомления заблокированы. Разрешите в настройках браузера.');
    return;
  }
  if (Notification.permission === 'default') {
    const permission = await Notification.requestPermission();
    if (permission !== 'granted') { alert('Нужно разрешить уведомления.'); return; }
  }
  try {
